# ===========================================================================
# Simulation Study: Cross-Fitting vs Non-CF TMLE — Survival Outcome
# ===========================================================================
#
# Complex DGP with genuine nonlinearities to demonstrate cross-fitting's
# value when SuperLearner with aggressive learners is used.
# See sim_cf_binary.R for full DGP documentation.
#
# Survival: Y(t) is absorbing — once Y=1, stays 1. DGP generates
# time-specific hazards; true values are cumulative incidence.
#
# 4-arm comparison:
...
#
# Usage:
#   source("longy/simulations/sim_cf_survival.R")
...
generate_data <- function(n, seed, scenario = "alternative") {
  ...
  L2_prev <- rep(0L, n)
  A_prev  <- rep(0L, n)
  Y_true  <- rep(0L, n)   # absorbing: once 1, stays 1
  alive   <- rep(TRUE, n)
  ...
  for (t in 0:4) {
    ...
    # --- Outcome (survival: absorbing binary) ---
    Y <- rep(NA_integer_, n)
    unc <- idx[C[idx] == 0L]
    if (length(unc) > 0) {
      # Already-event subjects stay at 1
      already_event <- unc[Y_true[unc] == 1L]
      at_risk       <- unc[Y_true[unc] == 0L]

      Y[already_event] <- 1L

      if (length(at_risk) > 0) {
        lp_h <- -2.5 + 0.2 * W1[at_risk] + 0.15 * W1[at_risk]^2 +
          0.3 * L1[at_risk] + 0.2 * L2[at_risk] +
          beta_YA * A[at_risk]
        if (scenario != "null") {
          lp_h <- lp_h + 0.15 * A[at_risk] * L1[at_risk]
        }
        Y[at_risk] <- rbinom(length(at_risk), 1, plogis(lp_h))
      }
    }
...
true_values <- function(scenario = "alternative", n_mc = 1e6, seed = 42) {
  ...
  for (regime in c("always", "never")) {
    ...
    L1_prev <- rep(0, n_mc)
    A_prev  <- rep(0, n_mc)
    surv    <- rep(1, n_mc)  # survival probability per subject (starts at 1)
    ...
    for (t in 0:4) {
      ...
      # Hazard at time t
      lp_h <- -2.5 + 0.2 * W1 + 0.15 * W1^2 + 0.3 * L1 + 0.2 * L2 +
        beta_YA * a_val
      if (scenario != "null") {
        lp_h <- lp_h + 0.15 * a_val * L1
      }
      h_t <- plogis(lp_h)

      # Cumulative incidence: P(Y(t)=1) = 1 - prod(1 - h(s)) for s=0..t
      surv <- surv * (1 - h_t)
      means[t + 1] <- mean(1 - surv)

      # For L1 feedback: need to draw Y for subjects still at risk
      # (L1 depends on A_prev, which is deterministic under regime, so
      # Y doesn't feed back into L1 in this DGP — we just need L1)
      L1_prev <- L1
...
run_simulation <- function(n = 1000, n_sim = 500, scenario = "alternative",
  ...
  cat(sprintf("\nRunning %d CF-comparison TMLE simulations (survival, n=%d, %s, %d cores)\n",
...
run_all <- function(n = 1000, n_sim = 500, seed_start = 1, n_cores = 10) {
  ...
  print_summary(
    sprintf("ALTERNATIVE (survival) -- ATE ~ %s", ate_label),
    regime_alt, ate_alt
  )
  ...
  print_summary(
    "NULL (survival) -- ATE = 0 at all times",

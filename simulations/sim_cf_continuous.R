# ===========================================================================
# Simulation Study: Cross-Fitting vs Non-CF TMLE — Continuous Outcome
# ===========================================================================
#
...
#
# Usage:
#   source("longy/simulations/sim_cf_continuous.R")
...
generate_data <- function(n, seed, scenario = "alternative") {
  ...
  for (t in 0:4) {
    ...
    unc <- idx[C[idx] == 0L]
    if (length(unc) > 0) {
      mu_Y <- 2 + 0.3 * W1[unc] + 0.2 * W1[unc]^2 +
        0.5 * L1[unc] + 0.3 * L2[unc] +
        beta_YA * A[unc] +
        0.3 * sin(W3[unc] * pi)
      if (scenario != "null") {
        mu_Y <- mu_Y + 0.2 * A[unc] * L1[unc]
      }
      Y[unc] <- mu_Y + rnorm(length(unc), 0, 1)
    }
...
true_values <- function(scenario = "alternative", n_mc = 1e6, seed = 42) {
  ...
  for (regime in c("always", "never")) {
    ...
    for (t in 0:4) {
      ...
      # E[Y(t)] under regime — direct expected value (no noise term)
      mu_Y <- 2 + 0.3 * W1 + 0.2 * W1^2 + 0.5 * L1 + 0.3 * L2 +
        beta_YA * a_val + 0.3 * sin(W3 * pi)
      if (scenario != "null") {
        mu_Y <- mu_Y + 0.2 * a_val * L1
      }
      means[t + 1] <- mean(mu_Y)
...
run_simulation <- function(n = 1000, n_sim = 500, scenario = "alternative",
  ...
  cat(sprintf("\nRunning %d CF-comparison TMLE simulations (continuous, n=%d, %s, %d cores)\n",
...
run_all <- function(n = 1000, n_sim = 500, seed_start = 1, n_cores = 10) {
  ...
  print_summary(
    sprintf("ALTERNATIVE (continuous) -- ATE ~ %s", ate_label),
    regime_alt, ate_alt
  )
  ...
  print_summary(
    "NULL (continuous) -- ATE = 0 at all times",

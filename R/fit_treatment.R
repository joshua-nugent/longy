...
#'   \code{cvControl} (e.g. \code{list(cvControl = list(stratifyCV = TRUE))}).
#'   \code{cvControl$V} sets the number of CV folds when
#'   \code{adaptive_cv = FALSE}; specifying \code{V} with
#'   \code{adaptive_cv = TRUE} is an error. Other elements (e.g.
#'   \code{method = "method.CC_LS"}) are passed directly to SuperLearner.
#' @param adaptive_cv Logical. Use adaptive CV fold selection based on effective
#'   sample size. When TRUE (default), the number of CV folds is chosen
#'   automatically. When FALSE, uses \code{sl_control$cvControl$V} if
#'   specified, or 10.
...
#'   Default FALSE (sequential SuperLearner). Forced to FALSE inside parallel
#'   workers to prevent nested parallelism.
...
fit_treatment <- function(obj, regime = NULL, covariates = NULL, learners = NULL,
  ...
  one_timepoint <- function(i) {
    ...
    if (n_risk == 0) {
      return(list(result = NULL, sl_info = NULL, n_marginal = 0L,
                  ...
                  msg = if (!parallel) sprintf("  g_A time %d: 0 at risk, skipping", tt) else NULL))
    }
    ...
    # Build verbose message (suppressed inside parallel workers)
    msg <- NULL
    if (!parallel && verbose) {
  ...
  if (verbose && parallel)
    .vmsg("  g_A: dispatching %d time points...", length(time_vals))
  ...
  if (!any(non_null)) {
    ...
    if (n_marginal > 0) {
      ...
      reason_str <- paste(sprintf("%s(%d)", names(reason_tbl), reason_tbl),
                          collapse = ", ")
      warning(sprintf(
        "g_A: marginal fallback at %d/%d time points [%s]. Run sl_diagnostics() for details.",
